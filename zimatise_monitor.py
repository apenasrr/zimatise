...
def check_project_in_monitor(folder_path_project: str, file_path_monitor: str):
    ...
    serie_bolean = df['project_path'].isin([folder_path_project])
    exist = any(serie_bolean)
...
def add_project_in_monitor(folder_path_project, file_path_monitor):
    ...
    # add project
    new_line = [dt_start, folder_name_project, folder_path_project,
                0, 0, 0, 0, 0, 0, 0, 0, 0, "", ""]
...
def get_flag_rule(flag_name: str) -> dict:
    ...
    flag_rules = {'was_started': was_started,
                  'to_zip': to_zip,
                  'to_report': to_report,
...
def process_flag_project(flag_rule: dict) -> bool:
    ...
    list_serie_bolean = \
        utils.get_series_bolean_by_df_filter(df, dict_=flag_rule)
    mask = utils.serie_bolean_mult_list(list_serie_bolean)
...
def check_and_add_new_project(folder_path_project,
                              file_path_monitor,
                              max_path):
    ...
    project_in_monitor = check_project_in_monitor(folder_path_project,
                                                  file_path_monitor)
    if project_in_monitor:
        print('-= Zimatise - Projects Monitor =-')
        # check_project_was_started
        project_was_started = check_flag_project(folder_path_project,
                                                 flag_name='was_started')
        ...
        if project_was_started:
            ...
            while True:
                ...
                if hit_max_path:
                    ...
                    choose_jump = ask_correct_or_jump(folder_path_project,
                                                      list_file_path_long)
                    if choose_jump:
...
def confirm_reencode_auth(folder_path_project):
    ...
    need_auth_to_reencode = check_flag_project(folder_path_project,
                                               flag_name='auth_encode')
