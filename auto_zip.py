...
try:
    ...
    utils.add_path_script_folders(['Zipind', 'mass_videojoin'])
    import zimatise_monitor
    import zipind_core
...
def process_zipind(project_path):
    ...
    folder_path_project_process = \
        utils.get_folder_path_project_process(project_path)
    folder_path_project_output = \
        utils.get_folder_path_project_output(folder_path_project_process)

    try:
        zipind_core.zipind(path_dir=project_path,
...
def show_projects_to_zip(list_project_path):
    ...
    str_header = ('Zipind - From a folder, ' +
...
def main():
    ...
    while True:
        list_project_path = \
            utils.get_list_project_to_process(file_path_monitor,
                                              flag_rule)
        utils.clean_cmd()
        ...
        if success:
            ...
            zimatise_monitor.set_stage_project(project_path,
                                               stage_name='2_auto_zip',
                                               stage_value=1)
        else:
            pass
